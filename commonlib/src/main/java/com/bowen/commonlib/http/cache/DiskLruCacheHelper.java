package com.bowen.commonlib.http.cache;
...
public class DiskLruCacheHelper {
    ...
    private DiskLruCache generateCache(Context context, File dir, int maxCount) throws IOException {
        ...
        int appVersion = context == null ? DEFAULT_APP_VERSION : ApplicationUtils.getVersionCode();
    ...
    private DiskLruCache generateCache(Context context, String dirName, int maxCount) throws IOException {
        DiskLruCache diskLruCache = DiskLruCache.open(
                ...
                ApplicationUtils.getVersionCode(),
                DEFAULT_VALUE_COUNT,
                maxCount);
        return diskLruCache;
    }
    ...
    public InputStream get(String key) {
        try {
            DiskLruCache.Snapshot snapshot = mDiskLruCache.get(MD5Util.hashKeyForDisk(key));
