...
public class CameraSurfaceView extends SurfaceView implements SurfaceHolder.Callback, Camera.AutoFocusCallback {
    ...
    private void setCameraParams(int width, int height) {
        ...
        for (Camera.Size size : pictureSizeList) {
            Log.i(TAG, "摄像头支持的分辨率：" + " size.width=" + size.width + "  size.height=" + size.height);
        }
        ...
        Log.e(TAG, "我们选择的摄像头分辨率：" + "picSize.width=" + picSize.width + "  picSize.height=" + picSize.height);
        ...
        for (Camera.Size size : previewSizeList) {
            Log.i(TAG, "摄像支持可预览的分辨率：" + " size.width=" + size.width + "  size.height=" + size.height);
        }
        Camera.Size preSize = getBestSupportedSize(previewSizeList, ((float) height) / width);
        if (null != preSize) {
            Log.e(TAG, "我们选择的预览分辨率：" + "preSize.width=" + preSize.width + "  preSize.height=" + preSize.height);
        ...
        mCamera.setErrorCallback(new Camera.ErrorCallback() {
            ...
            public void onError(int error, Camera camera) {
                String error_str = "摄像头异常，请检查摄像头权限是否应许";
                switch (error) {
    ...
    public boolean openCamera(int camera_id) {
        Log.i(TAG, "openCamera id = " + camera_id);
        try {
