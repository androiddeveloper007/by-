...
public class FileUtil {
    ...
    public static String getAppFileSaveRootDri(){
        if(TextUtils.isEmpty(filePath)){	// 如果无外置 SD卡， 优先获取内置SD卡
            filePath = getExternalStorageDirectory(CommonLibApplication.getCommonLibContext());
        }

        if(TextUtils.isEmpty(filePath)){	// 如果都没有，才使用机身内存
            filePath = getInternalFileDirectory(CommonLibApplication.getCommonLibContext());
        }
