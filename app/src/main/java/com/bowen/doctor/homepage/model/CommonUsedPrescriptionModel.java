...
public class CommonUsedPrescriptionModel extends BaseModel {

    public CommonUsedPrescriptionModel(Context mContext) {
        super(mContext);
    }

    public void loadData(int index, int pageSize, final HttpTaskCallBack<CommonUsedPrescriptionRecord> mListener) {
        ...
        networkRequest.requestSRV(HttpContants.CMD_GET_PRESCRIPTION_LIST, "", new HttpTaskCallBack<Object>() {
            @Override
            public void onSuccess(HttpResult<Object> httpResult) {
                HttpResult<CommonUsedPrescriptionRecord> result = new HttpResult<>();
                result.copy(httpResult);
                CommonUsedPrescriptionRecord record = GsonUtil.fromJson(GsonUtil.toJson(httpResult.getData()), CommonUsedPrescriptionRecord.class);
                if (record != null) {
