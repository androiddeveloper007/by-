package com.bowen.gallery.view;
...
import com.bowen.gallery.R;
import com.bowen.gallery.adapter.ImageFolderAdapter;
import com.bowen.gallery.adapter.ImageListAdapter;
import com.bowen.gallery.model.LocalMedia;
import com.bowen.gallery.model.LocalMediaFolder;
import com.bowen.gallery.utils.FileUtils;
import com.bowen.gallery.utils.GridSpacingItemDecoration;
import com.bowen.gallery.utils.ImageDataUtil;
import com.bowen.gallery.utils.LocalMediaLoader;
...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    public static void start(Activity activity, int maxSelectNum, int mode, boolean isShow, boolean enablePreview, boolean enableCrop,int requestCode) {
        Intent intent = new Intent(activity, ImageSelectorActivity.class);
        intent.putExtra(EXTRA_MAX_SELECT_NUM, maxSelectNum);
        intent.putExtra(EXTRA_SELECT_MODE, mode);
        intent.putExtra(EXTRA_SHOW_CAMERA, isShow);
...
//        this.requestCode = getIntent().getIntExtra(REQUEST_CODE, 0);
    ...
    public void startPreview(List<LocalMedia> previewImages, int position) {
        ImageDataUtil.getInstance().setImages(previewImages);
    ...
    public void onResult(ArrayList<String> images) {
        Intent intent = new Intent().putStringArrayListExtra(REQUEST_OUTPUT, images);
